...
// Éléments DOM
const formulaire = document.querySelector('#trackingForm');
const champCode = document.querySelector('#trackingCode');
const sectionResultat = document.querySelector('#resultat-section');
const sectionNonTrouve = document.querySelector('#non-trouve-section');
// Initialisation
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (sectionNonTrouve)
        sectionNonTrouve.style.display = 'none';
});
async function rechercherColis(e) {
    e.preventDefault();
    const codeRecherche = champCode === null || champCode === void 0 ? void 0 : champCode.value.trim();
    if (!codeRecherche)
        return;
    try {
        ...
        const tousLesColis = await response.json();
...
function afficherColisDetails(colis) {
    ...
    if (elementCode)
        elementCode.textContent = colis.id;
    ...
    if (nomExpediteur)
        nomExpediteur.textContent = colis.expediteur.nom;
    if (localisationExpediteur)
        localisationExpediteur.textContent = colis.expediteur.localisation;
    if (telephoneExpediteur)
        telephoneExpediteur.textContent = colis.expediteur.telephone;
    ...
    if (nomDestinataire)
        nomDestinataire.textContent = colis.destinataire.nom;
    if (localisationDestinataire)
        localisationDestinataire.textContent = colis.destinataire.localisation;
    if (telephoneDestinataire)
        telephoneDestinataire.textContent = colis.destinataire.telephone;
    ...
    if (poidsColis)
        poidsColis.textContent = `Poids: ${colis.details.poids}`;
    if (typeColis)
        typeColis.textContent = `Type: ${colis.details.type}`;
    if (prixColis)
        prixColis.textContent = `Prix: ${colis.details.prix}`;
}
function creerElementTimeline(item) {
...
function afficherColisNonTrouve() {
    if (sectionResultat)
        sectionResultat.style.display = 'none';
    if (sectionNonTrouve)
...
function afficherErreurRecherche() {
    ...
    if (sectionNonTrouve) {
        ...
        if (message)
            message.textContent = 'Impossible de se connecter au serveur. Vérifiez votre connexion internet.';
    }
}
async function afficherCarteColis(colis) {
    try {
